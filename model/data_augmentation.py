...
def generate_mask(valences, adj_mat, color, real_n_vertices, node_in_focus, check_overlap_edge, new_mol):
    ...
    for neighbor in range(real_n_vertices):
        if neighbor != node_in_focus and color[neighbor] < 2 and not \
        check_adjacent_sparse(adj_mat, node_in_focus, neighbor)[0]:
...
def update_one_step(overlapped_edge_features, distance_to_others, node_sequence, node_in_focus, neighbor, edge_type,
                    edge_type_masks, valences, incremental_adj_mat, color, real_n_vertices, graph, edge_type_labels,
                    local_stop,
    ...
    distances = [(start, node, params["truncate_distance"]) if d > params["truncate_distance"] else (start, node, d)
...
def construct_incremental_graph(dataset, edges, max_n_vertices, real_n_vertices, node_symbol, params, initial_idx=0):
    ...
    if params["generation"] not in [0,
                                    3]:  # in this way is returns all [] when the model is set to generation and reconstruction
        return [], [], [], [], [], [], [], [], []
    ...
    while len(queue) > 0:
        ...
        update_one_step(overlapped_edge_features, distance_to_others, node_sequence, node_in_focus, None, None,
                        edge_type_masks,
                        valences, incremental_adj_mat, color, real_n_vertices, graph, edge_type_labels, local_stop,
                        edge_masks,
                        edge_labels, True, params, params["check_overlap_edge"], new_mol, up_to_date_adj_mat, keep_prob)
        color[node_in_focus] = 2

    return incremental_adj_mat, distance_to_others, node_sequence, edge_type_masks, edge_type_labels, local_stop, \
