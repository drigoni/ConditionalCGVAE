...
class ChemModel(object):
    ...
    def run_epoch(self, epoch_name: str, epoch_num, data, is_training: bool):
        ...
        batch_iterator = ThreadedIterator(self.make_minibatch_iterator(data, is_training),
                                          max_queue_size=self.params['batch_size'])  # self.params['batch_size'])
        ...
        with self.graph.as_default():
            for epoch in range(1, self.params['num_epochs'] + 1):
                if self.params['generation'] == 0:
                    ...
                    print(
                        "\r\x1b[K Train loss: %.5f | Edge loss: %.5f | Node loss: %.5f | KL loss: %.5f | QED loss: %.5f | instances/sec: %.2f" %
                        (loss, mean_edge_loss, mean_node_loss, mean_kl_loss, mean_qed_loss, instance_per_sec))
                    ...
                    print(
                        "\r\x1b[K Valid loss: %.5f | Edge loss: %.5f | Node loss: %.5f | KL loss: %.5f | QED loss: %.5f | instances/sec: %.2f" %
                        (loss, mean_edge_loss, mean_node_loss, mean_kl_loss, mean_qed_loss, instance_per_sec))
                    ...
                    print(
                        "\r\x1b[K Test loss: %.5f | Edge loss: %.5f | Node loss: %.5f | KL loss: %.5f | QED loss: %.5f | instances/sec: %.2f" %
                        (loss, mean_edge_loss, mean_node_loss, mean_kl_loss, mean_qed_loss, instance_per_sec))
                    exit(0)
